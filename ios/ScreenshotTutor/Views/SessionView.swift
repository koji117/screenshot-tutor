...
struct SessionView: View {
    ...
    private func exportButton(_ s: Session) -> some View {
        // Stage on tap (not on every render) and hand off to
        // `UIDocumentPickerViewController(forExporting:)` via
        // `DocumentExporter`. The picker copies both the markdown
        // file and the `attachments/` folder into the destination
        // the user picks, preserving the structure that the inline
        // ![[attachments/...]] wikilink depends on. ShareLink with
        // multiple items including a folder URL silently flattens
        // or drops the folder on "Save to Files" — UIDocumentPicker
        // does not.
        Button {
