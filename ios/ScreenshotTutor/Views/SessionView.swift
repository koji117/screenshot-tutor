...
struct SessionView: View {
    ...
    private func exportButton(_ s: Session) -> some View {
        // Re-stage on each render. Cost: a small markdown write plus
        // one JPEG copy when an image is present. iPad SSD eats this
        // easily and it keeps the share contents fresh as the
        // summary / breakdown / chat fill in.
        let imageURL = store.imageURL(for: s)
        if let bundle = try? MarkdownExport.stageSession(s, sourceImageURL: imageURL) {
            // Share the markdown file *and* the attachments folder as
            // separate items. iOS's "Save to Files" copies each one
            // into the chosen destination, so the user ends up with
            // <vault>/<filename>.md sitting alongside
            // <vault>/attachments/<filename>.jpg — which is exactly
            // what the inline ![[attachments/...]] wikilink expects.
            ShareLink(
                items: bundle.shareURLs,
                subject: Text(MarkdownExport.sessionFilename(s)),
